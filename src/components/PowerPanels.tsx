...
export function QuarantinePanel({
  ...
  async function restore(entry: QuarantineEntry) {
    ...
    try {
      ...
      await onRefresh();
      onNotice(copy.status.quarantineRestored);
    } catch (error) {
  ...
  async function remove(entry: QuarantineEntry) {
    ...
    try {
      ...
      await onRefresh();
      onNotice(copy.status.quarantineDeleted);
    } catch (error) {
      onError(errorMessage(error));
    }
  }
  ...
  async function openEntry(path: string) {
    try {
      ...
      onNotice(copy.status.fileManagerOpened);
    } catch (error) {
      onError(errorMessage(error));
    }
  }
  ...
  async function refresh() {
    try {
      await onRefresh();
      onNotice(copy.status.quarantineRefreshed);
    } catch (error) {
      onError(errorMessage(error));
    }
  }
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <button className="secondary-button" type="button" onClick={() => void refresh()} disabled={busy}><RefreshCw size={17} />{copy.actions.scan}</button>
      </div>
      {entries.length === 0 ? (
            ...
            <article key={entry.cleanup_id} className="surface p-5">
              <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
                ...
                <div className="flex shrink-0 flex-wrap gap-2">
                  <button className="secondary-button" type="button" onClick={() => void openEntry(entry.quarantine_path)} disabled={busy}><ExternalLink size={16} />{copy.actions.open}</button>
...
export function AnalysisPanel({ roots, requestedRoot, copy, onError, onNotice }: { roots: CacheNode[]; requestedRoot: string; copy: Copy; onError: (message: string) => void; onNotice: (message: string) => void }) {
  ...
  async function analyze() {
    ...
    try {
      ...
      onNotice(copy.status.analysisCompleted);
    } catch (error) {
      onError(errorMessage(error));
    } finally {
      setBusy(false);
      ...
      {!largest ? <div className="surface grid min-h-[220px] place-items-center p-8 text-muted">{copy.analysis.empty}</div> : (
        <>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <ItemList title={copy.analysis.largestFiles} items={files} copy={copy} onError={onError} onNotice={onNotice} />
            <ItemList title={copy.analysis.largestDirectories} items={directories} copy={copy} onError={onError} onNotice={onNotice} />
          </div>
...
function ItemList({ title, items, copy, onError, onNotice }: { title: string; items: LargestItem[]; copy: Copy; onError: (message: string) => void; onNotice: (message: string) => void }) {
  async function copyPath(path: string) {
    try {
      await navigator.clipboard.writeText(path);
      onNotice(copy.status.pathCopied);
    } catch (error) {
      onError(errorMessage(error));
    }
  }
    ...
    <section className="surface p-5"><h3 className="font-black text-text">{title}</h3><div className="mt-3 grid gap-2">
      {items.map((item) => <div key={item.full_path} className="rounded bg-panel2 p-3">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted"><span>{copy.treemap.safety[item.safety]}</span>{item.modified_at && <span>{copy.analysis.modified}: {formatDate(item.modified_at)}</span>}{item.inaccessible && <span className="text-warn">{copy.analysis.inaccessible}</span>}
          ...
          <button className="icon-button" type="button" title={copy.actions.open} onClick={() => void openItem(item.full_path)}><ExternalLink size={14} /></button>
        </div>
      </div>)}
    </div></section>
  );
