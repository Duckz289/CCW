                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <button
                      ...
                      className={`grid h-8 w-8 place-items-center rounded-full bg-panel/80 text-text transition hover:bg-panel ${
                        selected ? "text-accent" : node.safety === "Safe" ? "text-text" : "text-warn"
                      }`}
                      onClick={() => onToggleNode(node)}
                      aria-label={`${selected ? "Remove" : "Select"} ${localizeDynamicText(language, node.label)}`}
                    >
                    ...
                    {visibleChildren.map((child) => (
                      <button
                        ...
                        className={`min-w-0 p-3 text-left transition hover:bg-panel/70 ${safetyClass[child.safety]} ${
                          selectedPaths.has(child.path) ? "ring-2 ring-accent ring-offset-1 ring-offset-ink" : ""
                        }`}
